...
async function getMuxUsage(): Promise<ServiceDetail> {
  ...
  return {
    ...
    details: { tokenId: process.env.MUX_TOKEN_ID?.slice(0, 8) + "..." },
    usage: [
...
async function getResendUsage(): Promise<ServiceDetail> {
  ...
  return {
    ...
    usage: [{ label: "Emails Sent", value: 0, max: 3000, unit: "emails/mo", percentage: 0 }],
    lastUpdated: new Date().toISOString(),
  };
}
...
async function getAIUsage(): Promise<ServiceDetail> {
  try {
    ...
    const providerData: Record<
      string,
      { total: number; active: number; exhausted: number }
    > = {};
    const providers = ["gemini", "groq", "openrouter", "mistral", "cerebras"];

    for (const p of providers) {
      const keys =
        (aiConfig as unknown as Record<string, { isExhausted?: boolean }[]>)[p] || [];
...
async function getEsewaUsage(): Promise<ServiceDetail> {
  try {
    ...
    return {
      ...
      details: { accountNumber: esewaNumber },
      usage: [],
      lastUpdated: new Date().toISOString(),
    };
  } catch {
...
/** All platform service usage/health metrics (env + config derived). */
export async function getAllServicesUsage(): Promise<{
  ...
  return {
    services: [mux, livekit, cloudinary, resend, ai, mongodb, esewa],
    lastUpdated: new Date().toISOString(),
  };
}
