...
/** PATCH /api/mobile/admin/quiz-management/[id] — edit topic / toggle active. */
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (typeof payload.subject === "string") updates.subject = normalizeQuizText(payload.subject);
    if (typeof payload.topic === "string") updates.topic = normalizeQuizText(payload.topic);
    ...
    return NextResponse.json({
      topic: { ...serializeTopic(topic), questionCount, sessionCount },
    });
  } catch (error: unknown) {
    console.error("PATCH /api/mobile/admin/quiz-management/[id] error:", error);
    if (typeof error === "object" && error !== null && "code" in error && error.code === 11000) {
      return NextResponse.json({ error: "That quiz topic already exists." }, { status: 409 });
    }
    return NextResponse.json({ error: "Failed to update quiz topic." }, { status: 500 });
  }
}

/** DELETE /api/mobile/admin/quiz-management/[id] — only if it has no history. */
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    console.error("DELETE /api/mobile/admin/quiz-management/[id] error:", error);
    return NextResponse.json({ error: "Failed to delete quiz topic." }, { status: 500 });
  }
}
